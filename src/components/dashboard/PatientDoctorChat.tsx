        ...
        <div className="w-1/3 border-r">
          <div className="p-3 border-b">
            <div className="relative">
              <input className="w-full px-3 py-2 bg-muted/50 rounded-md pl-8" placeholder="Search conversations..." />
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 absolute left-2 top-3 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <ScrollArea className="h-[calc(600px-110px)]">
            <div className="divide-y">
              {contacts.map(contact => <div key={contact.id} className={cn("p-3 hover:bg-muted/50 cursor-pointer transition-colors", activeContact === contact.id && "bg-muted")} onClick={() => setActiveContact(contact.id)}>
                  <div className="flex items-start gap-3">
                    <div className="relative">
                      ...
                      <span className={cn("absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-background", getStatusColor(contact.status))} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-baseline mb-1">
                        <h3 className="font-medium truncate text-lg">{contact.name}</h3>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        {contact.unreadCount > 0 && <Badge variant="default" className="ml-2 h-5 min-w-5 px-1 rounded-full">
                            {contact.unreadCount}
                          </Badge>}
                      </div>
                    </div>
        ...
        <div className="flex-1 flex flex-col">
          {activeContact ? <>
              ...
              <ScrollArea className="flex-1 p-4">
                ...
                const showDateSeparator = index === 0 || formatDate(message.timestamp) !== formatDate(conversations[activeContact][index - 1].timestamp);
                return <React.Fragment key={message.id}>
                        {showDateSeparator && <div className="flex justify-center my-2">
                            <span className="text-xs bg-muted px-2 py-1 rounded-full text-muted-foreground">
                              {formatDate(message.timestamp)}
                            </span>
                          </div>}
                        <div className={cn("flex", message.sender === "patient" ? "justify-end" : "justify-start")}>
                          <div className={cn("max-w-[80%] rounded-lg p-3", message.sender === "patient" ? "bg-primary text-primary-foreground" : "bg-muted")}>
                            <p>{message.content}</p>
                            {message.attachments && message.attachments.length > 0 && <div className="mt-2 space-y-1">
                                {message.attachments.map((attachment, i) => <div key={i} className="flex items-center gap-1 text-xs">
                            ...
                            <div className={cn("text-xs mt-1 flex justify-end", message.sender === "patient" ? "text-primary-foreground/70" : "text-muted-foreground")}>
                              {formatTime(message.timestamp)}
                              {message.sender === "patient" && <span className="ml-1">
                                  {message.read ? "✓✓" : "✓"}
                                </span>}
                            </div>
                          </div>
              ...
              <div className="p-3 border-t">
                {attachments.length > 0 && <div className="mb-2 p-2 bg-muted/30 rounded-md">
                    <div className="text-xs text-muted-foreground mb-1">Attachments</div>
                    <div className="flex flex-wrap gap-2">
                      {attachments.map((file, index) => <div key={index} className="flex items-center gap-1 bg-muted px-2 py-1 rounded-full text-xs">
                          <PaperclipIcon className="h-3 w-3" />
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="icon" className="shrink-0" onClick={() => document.getElementById('file-upload')?.click()}>
                    <input id="file-upload" type="file" multiple className="hidden" onChange={handleFileChange} />
                    <PaperclipIcon className="h-5 w-5" />
                  </Button>
                  <Textarea value={messageText} onChange={e => setMessageText(e.target.value)} placeholder="Type your message..." className="min-h-10 resize-none" onKeyDown={e => {
                if (e.key === 'Enter' && !e.shiftKey) {
                  e.preventDefault();
                  handleSendMessage();
                }
              }} />
                  <Button className="shrink-0" onClick={handleSendMessage} disabled={messageText.trim() === '' && attachments.length === 0}>
